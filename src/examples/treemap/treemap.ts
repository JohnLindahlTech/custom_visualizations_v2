...
const vis: TreemapVisualization = {
  ...
  update: function (data, element, config, queryResponse) {
    ...
    const format =
      formatType(measure.value_format) || ((s: any): string => s.toString());
    ...
    const treemap = d3
      .treemap<{ name: string; children: any[]; depth: number }>()
    ...
    const breadcrumb = svg.append("text").attr("y", -5).attr("x", 4);
    ...
    const cell = svg
      ...
      .on("mouseleave", (d) => {
        breadcrumb.text("");
        svg.selectAll("g.node rect").style("stroke", (d) => {
          return null;
        });
      });
    ...
    cell
      ...
      .style("fill", (d: any) => {
        ...
        const ancestor: string = d
          .ancestors()
          .map((p: any) => p.data.name)
          .slice(-2, -1)[0];
        const colors: any[] = [color(ancestor), "#ddd"];
        const scale = d3.scaleLinear().domain([1, 6.5]).range(colors);
