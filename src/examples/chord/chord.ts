...
const vis: ChordVisualization = {
  ...
  update(data, element, config, queryResponse) {
    ...
    // Set color scale
    const colorScale: d3.ScaleOrdinal<string, d3.ColorSpaceObject> =
      d3.scaleOrdinal();
    ...
    const color: d3.ScaleOrdinal<string, d3.ColorSpaceObject> =
      colorScale.range(config.color_range);
    ...
    const ribbons = svg
      ...
      .on("mouseenter", (d: any) => {
        tooltip.html(
          this.titleText(matrix.nameByIndex, d.source, d.target, valueFormatter)
        );
      })
    ...
    groupText
      ...
      .attr(
        "startOffset",
        (d: any, i: number) =>
          (groupPathNodes[i].getTotalLength() - thickness * 2) / 4
      )
    ...
    groupText
      .filter(function (this: SVGTextElement, d: any, i: number) {
        return (
          groupPathNodes[i].getTotalLength() / 2 - 16 <
